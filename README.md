...
- **消耗气泡不再自动关闭。**
  删除了 `showCostBubble` 中基于 `turnCostCloseMs` 的定时关闭逻辑，现在气泡只在用户点击时关闭。

- **重构了 `refresh()` 中的余额刷新逻辑**，区分三种情况分别处理：
  - 币种切换：直接同步显示新值，不做滚动动画。
  - 手动刷新（点击组件）：如果显示值与最新值不同，则播放数字滚动动画。
  - 后台自动刷新：只更新状态，不弹气泡、不滚动数字；`shown` 保持旧值，仅在首次加载时同步。

- **新增 `clickDelay` 控制点击后的滚动时机**：
  - 气泡刚展开时，延迟约 550ms 等文字淡入完成后再滚动数字；
  - 气泡已经展开时，立即滚动，不延迟。

## 改动原因

之前消耗气泡会在超时后自动关闭，用户可能还没看完信息气泡就消失了，体验不太好。
同时 `refresh()` 里动画和后台更新混在一起，导致手动点击和后台轮询的表现不一致。
这次改动让交互更明确：**气泡保持显示直到用户点击关闭，数字只在用户主动操作时才播放滚动动画。**
...
1. 点击组件展开气泡 → 确认数字在淡入后（约 0.55s）开始滚动。
2. 气泡保持展开并等待 → 确认不会自动关闭。
3. 点击气泡 → 确认气泡关闭。
4. 等待后台轮询刷新 → 确认数字不滚动、气泡不弹出。
5. 切换币种 → 确认数值立即更新，没有滚动动画。
